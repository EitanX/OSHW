...
const size_t MIN_BLOCK_SIZE = 1 << 7;
const size_t BASE_NUM_BLOCKS = 32;
const size_t HIGHEST_BLOCK_SIZE = 128 * 1024;
...
struct listOfBlocks {
    ...
    MallocMetadata *popSpecificBlockMetaData(MallocMetadata *block) {
    ...
    void insertBlockMetaData(MallocMetadata *blockMetaData) {
...
static void lazy_init() {
    if (heap_init) return;
    size_t initial_size = BASE_NUM_BLOCKS * order_to_size(MAX_ORDER);
    //first sbrk
    ...
    size_t max_size = order_to_size(MAX_ORDER);
    for (size_t i = 0; i < BASE_NUM_BLOCKS; ++i) {
        void *addr = (void *) ((intptr_t) heap_base + i * max_size);
    ...
    free_blocks = BASE_NUM_BLOCKS;
    free_bytes = BASE_NUM_BLOCKS * (order_to_size(MAX_ORDER) - sizeof(MallocMetadata));
...
static void *mmap_alloc(size_t size) {
    ...
    new_meta->next = mmap_head;
    new_meta->prev = nullptr;
    if (mmap_head) mmap_head->prev = new_meta;
    mmap_head = new_meta;
...
static void mmap_free(MallocMetadata *p_meta) {
    //delete from list
    if (p_meta->prev) p_meta->prev->next = p_meta->next;
    if (p_meta->next) p_meta->next->prev = p_meta->prev;
    if (mmap_head == p_meta) mmap_head = p_meta->next;

    size_t totalSize = p_meta->size + sizeof(MallocMetadata);
    //TODO: if munmap failed?
    munmap((void *) p_meta, totalSize);
    allocated_blocks--;
    allocated_bytes -= p_meta->size;
    meta_bytes -= sizeof(MallocMetadata);
}
...
void *srealloc(void *oldp, size_t size) {
    ...
    if (totalSize <= order_to_size(oldp_meta->order)) {
        ...
        if (highestShadowMergePossibleOrder(oldp_meta) >= totalOrder) {
            return specialFooFooFunction(oldp_meta, totalOrder);
        } else {
